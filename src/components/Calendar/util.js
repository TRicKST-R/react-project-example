const bindEvents = (events, date) => {
	...
	Object.keys(events).forEach(key => (boundEvents[key] = events[key].bind(null, date)))
...
const getModByDate = (mods, date, type) => {
	...
	mods
		.filter(mod => (mod.date ? mod.date.isSame(date, type) : null))
		.forEach(_mod => {
			modifier.date = _mod.date
			modifier.events = _mod.events
			if (Array.isArray(_mod.classNames)) modifier.classNames.push(..._mod.classNames)
		})
...
const getModsWithoutDate = mods => mods.filter(mod => !mod.date)
...
export const getMods = (mods, date, clsPrefix, type) => {
	...
	getModsWithoutDate(mods).forEach(mod => Object.assign(events, bindEvents(mod.events, date)))

	if (mod && mod.events) Object.assign(events, bindEvents(mod.events, date))
