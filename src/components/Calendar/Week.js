...
const makeWeekNumber = props => {
	if (!props.weekNumbers) return null
...
const renderWeekHeader = props => {
	if (!props.weekHeader) return null
...
const Week = props => {
	...
	if (!props.day) day = getModsByCompType('day', mods)
		...
		<div key="days" className={classnames(clsPrefix, clsMods)} {...events}>
			...
			<div className={classnames(`${clsPrefix}-days`)}>
				{daysOfWeek(props.date).map((date, i) => {
					...
					if (props.edges)
						outside = Boolean(
							props.edges.find((edge, j) => edge.isSame(date, 'month', 'week', 'year'))
						)
