...
import { FETCH_EMAIL_BOUNCED_USERS, UPDATE_EMAIL } from '../root/Graphql'
import Tooltip from '@material-ui/core/Tooltip'
import { push } from 'react-router-redux'
import MUIDataTable from 'mui-datatables'
import BouncedEmailsModal from '../components/Modals/BouncedEmailsModal'
...
class BouncedEmails extends Component {
	constructor(props) {
		super(props)
		this.state = {
			account: [],
	...
	makeAPICall = () => {
		...
		getChunk(FETCH_EMAIL_BOUNCED_USERS).then(data => {
			if (data && data.fetch_bounced_emails) {
				this.setState({ res: data.fetch_bounced_emails, refreshIsActive: true })
			} else {
				this.setState({ docsFetched: true, refreshIsActive: true })
				this.props.addSnack('GraphQL bad response on fetch email bounced')
			}
		})
	}
	...
	updateEmail = () => {
		...
		mutation(UPDATE_EMAIL, variable).then(data => {
			if (data && data.update_email && data.update_email.is_successfully_updated === 'true') {
				...
				this.setState({ emailOpened: false, checkedSendMail: false, waitingForResponse: false })
			} else {
...
const Styled = withStyles(styles)(BouncedEmails)
