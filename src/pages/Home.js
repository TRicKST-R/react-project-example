...
const Home = props => {
	...
	const search = () => {
		...
		getChunk(SEARCH_USERS, variables).then(data => {
			if (data.search_user && data.search_user.user) setUsers(data.search_user.user)
		...
		<div className={classes.root}>
			<Paper className={classes.search} elevation={5}>
				<InputBase
					...
					placeholder="email, username, gift_receipt_code, handle, user_id, account_id, apex_account_id, gift_id, object_id"
					value={input}
